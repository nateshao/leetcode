 ...
 * [题目]给定一 -个单向链表的头节点head，,节点的值类型是整型，再给定- -个
 * 整数pivot。实现- -个调整链表的函数，将链表调整为左部分都是值小于pivot
 * 的节点，中间部分都是值等于pivot的节点，右部分都是值大于pivot的节点。
 ...
 * 顺序与原链表中节点的先后次序-致。例如: 链表9 ->0->4->5->1, pivot=3。
 * 调整后的链表是0->1->9->4->5。在满 足原问题要求的同时，左部分节点从左到
 * 右为0、1。 在原链表中也是先出现0，后出现1;中间部分在本例中为空，不再.
 * 讨论;右部分节点从左到右为9、4、5。在原链表中也是先出现9，然后出现4,
 * 最后出现5。
 * 如果链表长度为N，时间复杂度请达到0 (N)，额外空间复杂度请达到0(1)。
 */
